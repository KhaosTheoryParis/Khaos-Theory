...
CREATE TABLE refund_operations_v3 (
  id TEXT PRIMARY KEY,
...
INSERT INTO refund_operations_v3 (
...
SELECT
  id, order_line_id, requested_quantity, refunded_quantity_before,
  amount, currency, stripe_idempotency_key, stripe_refund_id,
  status, NULL, pennylane_credit_note_id, credit_note_status,
  created_at, updated_at
FROM refund_operations;

DROP TABLE refund_operations;
ALTER TABLE refund_operations_v3 RENAME TO refund_operations;
...
CREATE INDEX idx_refund_operations_order_line_status
  ON refund_operations(order_line_id, status);
